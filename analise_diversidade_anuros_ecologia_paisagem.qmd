---
title: "analise diversidade anuros ecologia paisagem"
format: 
  html:
    toc: true
    toc-depth: 5
...
area_corpos_dagua <- buffers_lista %>%
  ...
  dplyr::group_by(Local) %>%
  dplyr::summarise(`Área total` = value %>% sum() * 10000) %>%
  dplyr::bind_rows(tibble::tibble(Local = c(3:4, 7:8, 10),
                                  `Área total` = 0)) %>%
  dplyr::arrange(Local) %>%
  dplyr::mutate(Local = fragmentos,
                `Área relativa` = `Área total` / area_total$Área)
...
dados_sps_registradas <- dados_sps %>%
  ...
  dplyr::group_by(Família) %>%
  dplyr::arrange(Família) %>%
  dplyr::group_modify(~ bind_rows(tibble(Espécie = NA), .x)) %>%
  dplyr::ungroup() %>% 
  dplyr::mutate(Família = dplyr::if_else(dplyr::row_number() == 1 | Família %>%
                                          dplyr::lag() != Família,
                                        Família,
                                        NA))
...
dados_sps_registradas_flex <- dados_sps_registradas %>%
  ...
  flextable::bg(i = ~!is.na(Família), bg = "gray") %>% 
  flextable::bg(i = ~is.na(Família), bg = "white") %>%
...
```{r}
modelo_riqueza_glm <- glm(Riqueza ~ PC1 + `Número de categorias` + `NDVI médio` + `Área de corpos d'água` + `Temp. média quart. seco` + `Prec. média quart. umido`,
                          data = df_riqueza_pc1,
...
```{r}
df_riqueza_pc1[c(3, 5, 7:10)]

df_riqueza_pc1 %>%
  tidyr::pivot_longer(names_to = "Variável",
                      values_to = "Valores preditores",
                      cols = c(3, 5, 7:10)) %>%
  dplyr::mutate(Variável = Variável %>% forcats::fct_relevel(df_riqueza_pc1[c(3, 5, 7:10)] %>% names())) %>% 
  ...
  geom_smooth(data = . %>% dplyr::filter(Variável %in% c("Temp. média quart. seco",
              ...
              se = FALSE,
              show.legend = FALSE) +
  scale_fill_manual(values = c("gold", "#49be12", "lightblue4", "orangered", "cyan4", "orange")) +
  scale_color_manual(values = c("orange4", "orangered4")) +
...
### Reduzindo a correlação

```{r}
pca_comp <- df_comp[c(3, 5)] %>%
  vegan::decostand(method = "standardize") %>%
  FactoMineR::PCA(graph = TRUE)
...
pc1_comp <- pca_comp$ind$coord[, 1]

pc1_comp

df_comp_pc1 <- df_comp %>%
  dplyr::mutate(PC1 = pc1_comp)
...
```{r}
modelo_comp <- glmmTMB::glmmTMB(`Dissimilaridade de Composição` ~ PC1 +
                                  `Dissimilaridade do número de categorias` +
...
tabela_glm_comp <- tibble::tibble(Preditor = c("PC1",
                                               "Dissimilaridade do número de categorias",
                                               "Dissimilaridade do NDVI médios",
                                               "Dissimilaridade da área de corpos d'água",
                                               "Dissimilaridade de Temp. seco",
...
```{r}
df_comp_pc1[c(2, 4, 6:9)]

df_comp_pc1 %>%
  tidyr::pivot_longer(names_to = "Variável",
                      values_to = "Dissimilaridade da paisagem",
                      cols = c(2, 4, 6:9)) %>%
  dplyr::mutate(Variável = Variável %>% forcats::fct_relevel(df_comp_pc1[c(2:4, 6:8)] %>% names())) %>%
...
dist_geo[dist_geo %>% upper.tri()] <- NA

dist_geo_vec <- dist_geo %>% 
...
lm(distancia_comp ~ dist_geo_vec) %>% 
  summary()

plot(dist_geo_vec, distancia_comp)
```
